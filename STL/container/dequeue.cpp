...
Eğer ekleme iki uçtan yapılmıyorsa bütün iteratörler ve referanslar invalidate oluyor. Eğer ekleme iki u.tan birinden yapılıyorsa iteratörler invalidate oluyor ancak referanslar invalidate olmuyor. 
Silme işlemi ortadan yapılıyorsa, bütün iteratörler ve referanslar invalidate oluyor. Eğer silme işlemi iki uçtan yapılıyorsa sadece silinen ögelerle iteratörler ve referanslar geçersiz hale geliyor. 

vector bellekte ardışık. Dolayısıyla vectordeki ilk ögenin adresi C dizileri üzerinde işlem yapan bir fonksiyona geçilebilir. Ancak dequeue için bu geçerli değil. dequeue içerisinde chunklar kendisi içinde ardışık.

vector<bool> diğer tipleri ile aynı specialization'a sahip değil. auto type deduction'da bazı problemlere yol açabiliyor. Bool yerine bir nested type tutuyor.
dequeue<bool> ise gerçekten bool tutuyor. 

dequeue stack container adaptöründe default container olarak kullanılıyor.  
